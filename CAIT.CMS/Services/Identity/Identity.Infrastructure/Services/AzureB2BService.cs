...
{
    public class AzureB2BService : IAzureB2BService
    {
        private readonly ILogger<AzureB2BService> _logger;
        ...
        public AzureB2BService(
            ILogger<AzureB2BService> logger,
        ...
        public async Task<(bool Success, LoginResponseDto? Response, string? Error, string? UserId)> ExchangeB2BTokenAsync(string azureAccessToken)
        {
        ...
        // for B2B Options : Organizations, Consumers and etc ...........

        //public async Task<(bool Success, LoginResponseDto? Response, string? Error, string? UserId)> ExchangeB2BTokenAsync(string azureAccessToken)
        //{
        //    var azureB2BSection = _config.GetSection("AzureAd");      // both are same
        //    var authority = $"{azureB2BSection["Instance"]}{azureB2BSection["TenantId"]}/v2.0";
        //    var audience = azureB2BSection["Audience"];
        //    var validIssuersTemplate = azureB2BSection.GetSection("ValidIssuers").Get<string[]>();
        ...
        //    try
        //    {
        //        principal = handler.ValidateToken(azureAccessToken, new TokenValidationParameters
        //        {
        //            ValidateIssuer = false, // مؤقتًا لتعريف tid
        ...
        //    catch (Exception ex)
        //    {
        //        _logger.LogError(ex, "Pre-validation failed");
        //        return (false, null, $"Invalid Azure B2B Token: {ex.Message}", null);
        //    }
        ...
        //    if (string.IsNullOrEmpty(oid) || string.IsNullOrEmpty(tid) || string.IsNullOrEmpty(email))
        //        return (false, null, "Required claims missing in token", null);

        //    // ✅ استبدال {tenantid} بـ tid الفعلي
        //    var resolvedIssuers = validIssuersTemplate.Select(i => i.Replace("{tenantid}", tid)).ToArray();

        //    // ✅ تحقق نهائي باستخدام الإعدادات الصحيحة
        //    try
        //    {
        //        principal = handler.ValidateToken(azureAccessToken, new TokenValidationParameters
        //        {
        ...
        //    catch (Exception ex)
        //    {
        //        _logger.LogError(ex, "Final validation failed");
        //        return (false, null, $"Issuer validation failed: {ex.Message}", null);
        //    }

        //    var objectId = Guid.Parse(oid);
        //    var tenantId = Guid.Parse(tid);

        //    var user = await _userManager.Users.FirstOrDefaultAsync(u => u.ExternalId == oid && u.AuthType == ApplicationUser.AuthenticationType.B2BGuest);
        ...
        //            UserName = email,
        //            Email = email,
        //            FirstName = jwtToken.Claims.FirstOrDefault(c => c.Type == "given_name")?.Value ?? "Guest",
        //            AuthType = ApplicationUser.AuthenticationType.B2BGuest,
        //            ExternalId = oid,
        //            AzureObjectId = objectId,
        //            AzureTenantId = tenantId,
        ...
        //        await _userManager.AddToRoleAsync(user, azureB2BSection["DefaultRole"] ?? "Guest");
        //    }
