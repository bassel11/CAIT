...
namespace Identity.Infrastructure.Services
{
    public class MfaService : IMfaService
    {
        ...
        public async Task<(bool Success, string? Message, string? Error)> ResendMfaCodeAsync(string userId)
        {
            var user = await _userManager.FindByIdAsync(userId);
            if (user == null) return (false, null, "User not found");

            if (!user.MFAEnabled)
                return (false, null, "MFA is not enabled for this user.");

            // We only resend codes for Email/SMS. TOTP is generated by the user's device.
            if (user.MFAMethod == MFAMethod.TOTP)
                return (false, null, "Resend not available for TOTP. Use your authenticator app.");

            if (user.MFAMethod == MFAMethod.Email)
            {
                // Optional: Rate Limiting Check here (e.g., allow once per minute)
                // if (user.LastMfaSent > DateTime.UtcNow.AddMinutes(-1)) return Error...

                var code = GenerateNumericCode(6);

                // Update User with new code
                user.MFACodeHash = HashCode(code, user.SecurityStamp);
                user.MFACodeExpiry = DateTime.UtcNow.AddMinutes(5); // 5 Minutes validity

                var result = await _userManager.UpdateAsync(user);
                if (!result.Succeeded)
                    return (false, null, "Failed to update user MFA code.");

                await _emailService.SendMfaCodeAsync(user.Email!, code);

                return (true, "Verification code resent successfully.", null);
            }

            return (false, null, "Unsupported MFA method for resend.");
        }
